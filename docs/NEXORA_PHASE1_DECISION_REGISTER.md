...
| R24 | **Advance and progress invoicing stay possible. The accepted-quantity ceiling binds only a line with at least one unit despatched.** A line with nothing despatched remains on the pre-existing order-status eligibility gate. | Deposits and milestone billing are normal Gulf trading practice — a customer PO for long-lead equipment is often part-paid before anything ships, and a ceiling that bound every line to accepted quantity would make that impossible. The ceiling exists to stop billing for goods the customer **refused**, not to stop billing before goods move. Once a line has despatched anything, accepted becomes the honest ceiling and the protection applies in full. Reversible in one line by dropping the despatch-activity clause; `A_line_with_nothing_despatched_stays_on_the_pre_existing_ordered_ceiling` is the test that changes if the owner overrules this. |
| R25 | **Delivered means confirmed-received, and it is carried as a second number rather than replacing despatched.** Despatched drives the over-shipment ceiling and goods issue; **accepted drives invoicing and fulfilment.** The ledger is derived, never stored. | A delivery note evidences that goods left; only a signed receipt evidences that the customer took them, and in a dispute only the second decides who is right. Collapsing them either bills for goods still on a lorry or refuses to bill for goods signed for last week. Carrying both also exposes the "went out, nobody confirmed" queue, which is otherwise invisible. Deriving rather than storing means there is no delivered-quantity column to drift out of agreement with the proofs. Consequence: **DELIVERED and DELIVERY_EXCEPTION are unreachable by an operator** — both are derived from what the customer accepted, so nobody can stamp a shipment delivered without evidence. |

| R26 | **Deferral list, decided under standing delegation while the owner was away (2026-08-10).** Each of these is *not built*, deliberately, and each is cheap to add later. Nothing here is abandoned — it is sequenced. | **Arabic/RTL** — already deferred by R6; the delivery note and quote PDF name it as a gap rather than half-rendering it. **ZATCA e-invoicing (FR-DLM-04)** — the document is capped against accepted quantity before it exists, so the seam is clean, but clearing needs credentials and a sandbox the owner has not supplied; building a serializer we cannot certify would produce confident, unverifiable output. **Carrier tracking API (FR-MAS-02)** — no carrier is contracted (decision D4 open); the manual path is complete and `TrackingSource` marks the seam, so adding an adapter later needs no schema change. **SMS and WhatsApp (FR-DLM-06)** — provider contracts, per-message cost and KSA sender-ID registration are unscoped; email works. **Stock count sessions (FR-INV-05)** — the variance report is built; a governed count document with blind counts and an approval step is a module, and a stock take conducted as independent postings is honest about what it is. **Invoice payment-due reminders (FR-SBF-01e)** — blocked on open decision D7, owned by Finance. **Vehicle scheduling, route planning and GPS tracking** — out permanently per R22, they belong to a TMS. **Claims, liability and corrective-action workflow** — out permanently per R23. |
| R27 | **Gate 9 is split: the code half will be completed; the certification half cannot be, and is not claimed.** | Buildable and being done: security hardening, tenant-isolation coverage, the row-level-security and pending-model guards, and performance work that is measurable locally. **Not buildable by any amount of engineering, and requiring the owner's money or signature:** PDPL data residency — none of Render, Vercel or Neon offers a Saudi region, so this is a re-platforming programme, not a sprint; real malware scanning — approximately $85/month on a product whose primary input is customer-supplied documents; 99.5% availability — structurally unreachable on a single instance; and backup with a **tested** restore — the document volume has none configured and the config file itself states that losing it is unrecoverable. Gate 9 will be reported as code-complete and certification-blocked, with those four named. Claiming otherwise would be the same defect this build has spent two days removing: an assertion with nothing behind it. |
| R28 | **Gate 10 is staged, not attempted.** | History migration, training, parallel run and pilot acceptance need real users on real infrastructure. Two are also blocked on prior decisions: the two-year quotation history load is refused by the closed commercial-case spine and needs the governed import path recorded as **E47**, and pilot acceptance needs a pilot environment with a live mailbox. The correct deliverable overnight is a documented, honest statement of what Gate 10 requires — not a simulation of having run it. |

| R29 | **A despatched shipment cannot be cancelled, and a despatched or proved shipment cannot be deleted. Refusal, not reversal — because no goods-return path exists to reverse truthfully.** Decided 2026-08-10 under standing delegation, narrowing FR-DLM-05. | Two defects, one root cause. **(1)** `DeliveryStatuses.Cancellable` listed DISPATCHED and IN_TRANSIT, and the "Mark Cancelled" button on the shipment screen changed a status and wrote a note. The goods issue and the lot consumption are made at shipment creation and `DeliveryConfirmationService` holds **no stock dependency at all**, so one click removed the quantity from the over-shipment ceiling and from the delivered ledger while the material was on a lorry — the order line became fully re-shippable against stock that had gone, and the cancelled run's lot consumptions stayed on the traceability record. Two files' own comments asserted the opposite ("reverses whatever it accrued"). **(2)** `ShipmentRepository.DeleteShipmentAsync` set `IsActive = false` with no status check, no proof-of-delivery check, no reason and no actor, on the `Shipments:Delete` permission — wiring-contract failure #9, since `Cancellable` had existed since Gate 7 precisely to say a confirmed shipment must not be reversed. **Reversal would be the better answer if it could be told truthfully, and it cannot.** No goods-receipt reversal exists anywhere in this codebase: nothing can say a particular despatch came back, into which warehouse, in what condition, on whose word; `MaterialLotConsumption` is append-only evidence of what left on a delivery note and a recall trace that can lose rows is not a recall trace; and a cancellation carries no return date, no receiving bay and no evidence, so putting the stock back would be the system asserting a physical event nobody witnessed. Building the return means new tables — a migration-owner decision, not a defect fix, and out of scope beside R23's exclusion of RMA and reverse logistics. **What an operator does instead:** a consignment the customer refused in full is a delivery outcome, not a cancellation — confirm it with zero accepted on every line and a reason of `REJECTED`, and the whole quantity stays outstanding for a re-supply or credit decision. **Named gap:** a consignment **lost in transit** has no receiving person, and `ConfirmAsync` requires a receiving contact name, so `LOST_IN_TRANSIT` is not cleanly recordable today; the carrier claim is out of scope per R23, but the commercial write-off route is not yet reachable and is owed. Same call Gate 5 made inbound, and the same call the "Rejected in this gate" note below already made for supplier orders. |

| R30 | **The below-floor margin control will be given a real floor, not deleted.** The floor is the awarded supplier landed unit cost already carried on `CustomerQuoteSourcingDecision`. Decided 2026-08-10 under standing delegation. | The Gate 2 audit found `PricingEngine.cs:248` declares `decimal? floor = null;`, never reassigns it, and writes that null to `line.FloorUnitPrice` — the only assignment to that property anywhere in the codebase. Every downstream filter (`BelowFloorGuard.cs:118,184`) therefore matches nothing, so `CheckQuoteSendAsync` always returns Clear and **no margin is ever compared to any threshold before a customer quote is released.** The approvals inbox entry, the deadline-buffer escalation and the careful FX conversion behind it are all unreachable. Wiring-contract failure #7 sitting on failure #1, and the suite pins the defect rather than catching it: `Release02CommercialLearningTests.cs:337` asserts `FloorUnitPrice` is *always null*. The contract says a control that cannot fire is worse than no control, which leaves two honest options — wire it or delete it. **Wire it.** This is a trading business whose margin is the entire product; selling below landed cost without a human saying yes is the single most expensive thing the system could quietly permit, and the authoritative floor already exists — `SupplierLandedUnitCost` on the awarded sourcing decision, the same number the customer price is derived from. Deleting the limb would be cheaper today and wrong by next quarter. **Consequence:** a quote line whose price falls below its awarded landed cost cannot be sent without approval, and `Release02CommercialLearningTests.cs:337` is the test that must be rewritten — it currently certifies the bug. |
| R31 | **A supplier's rejection must release the demand it was covering.** Decided 2026-08-10 under standing delegation. | The Gate 4 audit found that recording `AcknowledgementStatus = REJECTED` does not move the order off `SENT`, so it stays in `CommittedSupply` and `GetNetSourcingRequirementAsync` still nets its full quantity off the RFQ line — while the two SLA sweeps deliberately skip rejected orders. Three alarms silenced, coverage still counted, supplier refused: **the material is never bought and nothing anywhere says so.** Before the acknowledgement feature existed, a rejection could not be recorded and the escalation kept chasing, which was noisy but correct. The fix inverted a double-buy into an un-buy. This is worse than the defect it replaced because it is silent, and silence is what this build has spent three days removing. The demand must return to the sourcing pool and the case must show a visible gap, not a covered line. |
