...
export default function PerformancePage() {
  ...
  return (
    <PageShell title="Sales performance" subtitle="Verified results for the selected half-open reporting period." actions={<Stack direction="row" spacing={1}><TextField size="small" type="date" label="From" value={from} onChange={event => setFrom(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} /><TextField size="small" type="date" label="To" value={to} onChange={event => setTo(event.target.value)} error={!valid} slotProps={{ inputLabel: { shrink: true } }} /></Stack>}>
      ...
      <QueryState loading={query.isLoading} error={query.isError || !valid} empty={valid && !rows.length} onRetry={() => void query.refetch()} emptyText="No performance records exist in this period.">
        <ResponsiveTable label="Representative performance">
          <Table size="small">
            <TableHead><TableRow><TableCell>Representative</TableCell><TableCell align="right">Won</TableCell><TableCell align="right">Lost</TableCell><TableCell align="right">Conversion</TableCell><TableCell align="right">Pipeline</TableCell><TableCell align="right">Overdue leads</TableCell></TableRow></TableHead>
            <TableBody>{rows.map(row => <TableRow hover key={row.userId}><TableCell>{row.name}</TableCell><TableCell align="right">{row.wonQuotes}</TableCell><TableCell align="right">{row.lostQuotes}</TableCell><TableCell align="right">{row.conversionRate == null ? 'Insufficient data' : `${row.conversionRate.toFixed(1)}%`}</TableCell><TableCell align="right"><PipelineGroups groups={row.pipelineGroups} /></TableCell><TableCell align="right">{row.overdueLeads}</TableCell></TableRow>)}</TableBody>
          </Table>
        </ResponsiveTable>
      </QueryState>
    </PageShell>
