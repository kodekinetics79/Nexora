...
| Integration status / sync health | M1 | M3 | Outbox, health checks and handoff sync exist; no unified tenant/admin view | Persisted status projection and authorized replay only | Gate 3 passed: tenant connector status, provider acceptance receipts, fenced dispatch, backlog/retry/uncertainty/dead-letter/stale/difference evidence and fail-closed callback replay |
| Operational visibility | M2 | M3 | Procurement handoff external observations are non-authoritative | Surface source, freshness and authority; never infer fulfilment | Gate 3 passed: signed append-only observations, authoritative lifecycle source/freshness, rejected variance evidence and protected manual reference capture |
| Local-first AI governance | M3 | M3 | AI reservation/cost ledger and governed extraction exist | Close visibility and direct-provider-call gaps; external remains disabled by default | Gate 4 passed: canonical processing lineage, explicit OCR/path/cost status, tenant-bound provider governance, bounded fallback and authenticated journey evidence |
| Outcome Learning Studio | M2 | M3 | Commercial learning studio endpoint exists; approval/disable visibility incomplete | Review/approval surface over governed tenant memory | Gate 4 passed: persisted tenant-scoped signals with append-only approve, disable and rollback decisions, evidence snapshots, reasons, versions and idempotency |
| Performance / production certification | M2 | M3 | Existing benchmark and release evidence under `docs/nexora/evidence/` | Preserve baselines; record query, browser, security and migration evidence | Gate 5 code acceptance passed: 825 portable, 140 PostgreSQL and 38 authenticated browser scenarios passed with zero skips; production deployment remains blocked on external infrastructure verification recorded in `evidence/v1/gate-05.md` |
