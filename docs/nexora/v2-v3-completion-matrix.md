...
| V2.2 | Opportunity Priority and Next-Best Action | M2 | M4 shadow | In progress | M3 local deterministic, stage-aware shadow scoring; immutable evidence/recommendation/outcome/feedback; terminal-case exclusion; bounded reconciliation; owner/manager scope; RLS and lineage triggers are green. M4 remains blocked until separate currency-safe value, evidenced win likelihood, margin, customer quality, fulfilment confidence, effort, blocker and Expected Commercial Value components are persisted and visible. See `docs/nexora/releases/v2-gate-02-opportunity-priority-shadow.md`. |
...
- This is an M3 shadow foundation, not final M4 Gate 2 acceptance. The master-program component and Expected Commercial Value contract remains open, so Gate V2.3 must not begin.
