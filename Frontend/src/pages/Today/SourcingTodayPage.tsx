...
export default function SourcingTodayPage() {
  ...
  const query = useQuery({ queryKey: ['sourcing-today'], queryFn: () => supplierQuoteService.getInbox(), refetchInterval: 60_000, meta: { silenceGlobalError: true } });
  ...
  return <PageShell title="Sourcing today" subtitle={`${reviewRequired} Supplier Quote(s) need review; ${ready} are ready for comparison.`} actions={<Button variant="outlined" onClick={() => navigate('/procurement/rfqs/all?state=requires-sourcing')}>Open sourcing queue</Button>}>
    <QueryState loading={query.isLoading} error={query.isError} hasData={query.data !== undefined} updatedAt={query.dataUpdatedAt} empty={!rows.length} onRetry={() => void query.refetch()} emptyText="No Supplier Quote responses require sourcing attention.">
