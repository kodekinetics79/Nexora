...
/**
 * FR-MAS-03. The two numbers behind every material available date in the tenant.
 *
 * Blank means NOT CONFIGURED and no date is derived anywhere — deliberately, because a lead time
 * silently treated as zero produces a plausible date equal to the ETA on every shipment, and
 * somebody promises a customer against it.
 */
function PolicyDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const [refusal, setRefusal] = useState<string | null>(null);
  const [idempotencyKey] = useState(() => commandKey("inbound-policy"));
  ...
  const mutation = useMutation({
    mutationFn: () =>
      inboundShipmentService.updatePolicy({
    ...
    onError: (error) => {
      const message = errorMessage(error, "Could not save the lead times.");
      setRefusal(message);
      toast.error(message);
    },
  });
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          <TextField
            fullWidth
            type="number"
            label="Customs clearance (working days)"
            value={customsValue}
            error={!usable(customsValue)}
          ...
          <TextField
            fullWidth
            type="number"
            label="Putaway (working days)"
            value={putawayValue}
            error={!usable(putawayValue)}
          ...
          {policy.data?.modifiedBy && (
            <Typography variant="caption" color="text.secondary">
              ...
              {policy.data.modifiedOn
                ? ` on ${new Date(policy.data.modifiedOn).toLocaleString()}`
                : ""}
            </Typography>
          )}
        </Stack>
