...
public sealed class AiExternalProviderTrustService : IAiExternalProviderTrust
{
    ...
    /// <summary>
    /// The gate READS. It has no writer, deliberately.
    ///
    /// <para>It used to open with one: every evaluation called an auto-provisioner that set
    /// <c>IsEnabled</c>, <c>ExternalProcessingAllowed</c> and <c>EgressPolicy</c>, overwrote the
    /// purpose list, and minted a never-expiring grant authored by <c>AuthorizedByUserId = 0</c> —
    /// with no audit row and no <c>Version</c> increment. It was on by default, and
    /// <c>GetAsync</c> called it too, so merely OPENING a tenant's AI tab rewrote the governed
    /// row. An Owner who turned AI off through the front door (Owner + second factor + a written
    /// reason + an optimistic-concurrency check) had it turned back on by the next document, and
    /// the audit trail showed only the switching off.</para>
    ///
    /// <para>A governed row gets exactly one writer, and it is never the code path the row
    /// governs. Provisioning writes it; <c>PUT /api/platform/tenants/{id}/ai-policy</c> changes
    /// it; this class reads it and refuses. The dead-lettering the auto-provisioner was added to
    /// prevent is now handled where it belongs — a tenant is provisioned able to read
    /// documents.</para>
    /// </summary>
