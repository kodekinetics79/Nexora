...
function RecommendationEvidence({ item }: { item: OpportunityPriorityItem }) {
  ...
  return (
    <Box>
      <Button
        ...
        aria-expanded={open}
        aria-controls={`priority-evidence-${item.recommendationId}`}
        onClick={() => setOpen((current) => !current)}
      >
        {open ? 'Hide rationale' : 'Show rationale'}
      </Button>
      <Collapse in={open}>
        <Stack id={`priority-evidence-${item.recommendationId}`} component="ul" spacing={0.5} sx={{ pl: 2.5, my: 1 }}>
...
export default function SalesTodayPage() {
  ...
  const priorities = useQuery({
    queryKey: ['opportunity-priorities'],
    queryFn: opportunityPriorityService.getPriorities,
    retry: 1,
  });
  ...
  return <PageShell title="Sales today" subtitle={query.data?.scope === 'tenant' ? 'Team-wide commercial work that needs attention now.' : 'Your assigned commercial work that needs attention now.'}>
    ...
    <Stack spacing={1.5} sx={{ mb: 3 }}>
      ...
      {reconcile.isError && <Alert severity="error">Opportunity evidence could not be reconciled. Existing persisted priorities remain unchanged.</Alert>}
      {priorities.data && (
        <Stack spacing={0.25}>
          <Typography variant="caption" color="text.secondary">
            Scope: {priorities.data.accessScope} | Generated {formatDateTime(priorities.data.generatedAtUtc)}
          </Typography>
          <Typography variant="caption" color="text.secondary">
              ...
              <TableBody>
                {priorityItems.map((item) => (
                  <TableRow hover key={item.recommendationId}>
                    ...
                    <TableCell><Typography sx={{ fontWeight: 800 }}>{item.recommendedActionLabel}</Typography><RecommendationEvidence item={item} /></TableCell>
                    <TableCell><Typography variant="body2">{item.priorityBand} | Score {item.priorityScore}</Typography><Typography variant="caption" color="text.secondary">Completeness {percentage(item.completeness)} | Sample {item.sampleSize}</Typography></TableCell>
