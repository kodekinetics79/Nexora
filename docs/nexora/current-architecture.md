...
- Opportunity priority is a separate derived shadow projection anchored by tenant, Lead, Commercial Case, and Nexora Serial. Reconciliation runs in cursor-bounded batches, excludes terminal Quote/Order cases, and persists immutable evidence, recommendations, observed later outcomes, feedback, events, operations, and outbox records.
- Opportunity scoring uses deterministic unique exact-part coverage, decision-grade canonical customer identity, extraction confidence, deadline, and current commercial stage. Its separate commercial component snapshot records value, win-likelihood proxy, currency-qualified margin, urgency, customer-quality proxy, persisted current-revision line-resolution fulfilment confidence, and sourcing effort with sample size, confidence, freshness, and evidence.
- Expected Commercial Value is persisted only when every required input is present, remains explicitly currency-denominated and `shadow_unvalidated`, and is never compared across currencies in the tenant-wide queue. The current Product master does not carry cost currency, so its cost cannot satisfy the margin contract and the real resolver leaves ECV unavailable. Incomplete, stale, or superseded-revision line-resolution evidence also fails closed. Accuracy remains unavailable until an adequate later-outcome cohort exists.
...
- V2.3 pricing is shadow-only. The legacy RFQ price overwrite endpoint and agent tool fail closed; authoritative pricing remains the serializable, idempotent Supplier award to Customer Quote sourcing decision.
- RFQ Digital Twin scenarios use current ATP and current canonical, governed, same-currency Supplier Quote evidence. They expose allocations, cost provenance, delivery, validity, margin/risk availability, assumptions, confidence, approvals and evidence; no FX, stock cost, alternate approval or risk value is invented.
- Predictive price cohorts are tenant-scoped, exact-Product and exact-currency, trailing 24 months, with actual Customer Orders as wins. Same-Customer and quantity-comparable cohorts are preferred; sparse cohorts are withheld and chronological walk-forward MAPE is reported when at least four distinct order outcomes exist.
- Supplier negotiation is a shadow-only read/decision layer over the current canonical Supplier Quote revision. It evaluates deterministic bid quality and tenant-qualified comparison cohorts, then persists append-only evidence snapshots under separate history-view and negotiation-edit permissions. Blocking findings prevent preparation, corrected alternate review values are authoritative, and no recommendation can send, award or alter price.
- M4 remains blocked until immutable Digital Twin run snapshots, user override/outcome events, production calibration/drift controls, authoritative internal-stock cost currency and approved alternate evidence are implemented.
